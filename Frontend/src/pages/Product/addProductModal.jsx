...
const AddProductModal = ({
  ...
  const handleImageFileChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      // Validate file type - Cloudinary supports these formats
      const allowedTypes = [
        "image/jpeg",
        "image/jpg",
        "image/png",
      ...
      // Cloudinary has generous limits, but still good to check
      ...
      setImageFile(file);
      setCloudinaryPublicId(""); // Reset public ID for new upload
  ...
  const uploadImageFile = async () => {
    ...
    try {
      ...
      // Store Cloudinary public_id for potential deletion
      if (data.public_id) {
  ...
  // Function to delete image from Cloudinary if upload fails
  const deleteCloudinaryImage = async (publicId) => {
    try {
      if (!publicId) return;

      const response = await fetch(
        `${API_ROUTES.IMAGE_UPLOAD.DELETE_IMAGE}/${publicId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const AddProductDetails = async (newProduct) => {
    ...
    try {
      ...
      let imageUrl = newProduct.image;
      if (uploadMethod === "file" && imageFile) {
        uploadedImageData = await uploadImageFile();
      ...
      const productWithImage = {
        ...newProduct,
        image: imageUrl,
        // Optionally store Cloudinary public_id for future operations
        cloudinary_public_id: uploadedImageData?.public_id || null,
      };
      ...
      if (!response.ok) {
        // If product creation fails, cleanup the uploaded image
        if (uploadedImageData?.public_id) {
          await deleteCloudinaryImage(uploadedImageData.public_id);
        }
      ...
      // Cleanup uploaded image if product creation failed
      if (uploadedImageData?.public_id) {
        await deleteCloudinaryImage(uploadedImageData.public_id);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    const newProduct = {
      name,
      image: uploadMethod === "url" ? image : "", // Will be replaced with Cloudinary URL
      productDetails: description,
      category,
      price: Number(price),
      rating: Number(rating),
  ...
  const handleModalClose = async () => {
    // If there's an uploaded image but no product created, cleanup
    if (cloudinaryPublicId && uploadMethod === "file") {
